...
Clean folder-style category system with product browsing inside categories.
"""
...
def buying_menu_kb(coupons: list, free_coupon_count: int = 0, page: int = 0,
    ...
    """Build the shop menu — shows ONLY category folders.

    Categories are displayed as clean folder buttons.
    No stock info, no sold-out labels on folders.
    User clicks a folder → opens it → sees products inside.
    """
    buttons = []
    ...
    if categorized_data:
        ...
        for cat in categorized_data.get("categories", []):
            ...
            # Clean folder label — just name and product count
            if coupon_count > 0:
                label = f"📁 {cat_name} ({coupon_count})"
            else:
                label = f"📁 {cat_name}"
...
def category_coupons_kb(coupons: list, category_name: str, page: int = 0) -> InlineKeyboardMarkup:
    """Show products inside a specific category folder.

    Products show their individual price and stock status.
